...
namespace StarSim
{
    public class Renderer
    {
        ...
        {
            outX = (x + (float)CamPosX) * (float)scaling + control.Width / 2f;
            outY = (y + (float)CamPosY) * (float)scaling + control.Height / 2f;
        }
        ...
        public void Render(object sender,Graphics graphics)
        {
            ...
            */
            ...
            for (int iS = 0; iS < simulation.Stars.Length; iS++)
            {
                ...
                if (r < 0.01) r = 0.01f;
                if (starArray[iS] == simulation.SelectetStar)
                {
                ...
                if ((simulation.SelectetStar == starArray[iS] || starArray[iS].Marked) && showStarInfo || starArray[iS].Editing)
                {
                    posX += r;
                    posY += r;
                    ...
                    if (starArray[iS].Name.Length > 0) g.DrawString("" + ((starArray[iS].Name.Length > 0) ? starArray[iS].Name : String.Format("{0:X}", starArray[iS].Idx)), new Font("Consolas", 9), brush, new PointF(txtPosX, txtPosY += 15));
